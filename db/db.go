...
func InitPostgresDB() {
	...
	// create movies table in db if not already exist and update schema
...
func UpdateMovie(movie *Movie) (*Movie, error) {
	...
	res := db.Model(&movieToUpdate).Where("id = ?", movie.ID).Updates(movie)
	if res.RowsAffected == 0 {
...
func DeleteMovie(id string) error {
	...
	res := db.Where("id = ?", id).Delete(&deletedMovie)
	if res.Error != nil {
